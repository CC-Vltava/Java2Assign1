...
public class MovieAnalyzer {
	...
	public String noEmpty(String s) {
		...
		for (int i = 0; i < s.length(); i++) if (s.charAt(i) != ' ') ans.append(s.charAt(i));
		return ans.toString();
	}
...
//		int flag = 0;
		...
		while (scanner.hasNext()) {
			List<String> data = splitLine(scanner.nextLine()); this.datas.add(data);
		}
		scanner.close();
	}
	...
	public Map<Integer, Integer> getMovieCountByYear() {
		...
		for (List<String> item : this.datas) {
			String year = item.get(2); if (year.equals("")) continue; Integer ce = Integer.parseInt(year);
			if (ans.containsKey(ce)) ans.replace(ce, ans.get(ce), ans.get(ce) + 1);
			else ans.put(ce, 1);
		}
	...
	public Map<String, Integer> getMovieCountByGenre() {
		...
		for (List<String> item : this.datas) {
			...
			for (String eachGenre : genreInItem) {
				...
				if (ans.containsKey(genre)) ans.replace(genre, ans.get(genre), ans.get(genre) + 1);
				else ans.put(genre, 1);
			}
		}
		List<Map.Entry<String, Integer>> answer = new ArrayList<>(ans.entrySet());
		answer.sort((a, b) -> (a.getValue().equals(b.getValue()) ? a.getKey().compareTo(b.getKey()) : Integer.compare(-a.getValue(), -b.getValue())));
	...
	public void addStars(Map<List<String>, Integer> map, String a, String b) {
		...
		if (map.containsKey(ce)) map.replace(ce, map.get(ce), map.get(ce) + 1);
		else map.put(ce, 1);
	}
	...
	public Map<List<String>, Integer> getCoStarCount() {
		...
		for (List<String> item : this.datas) {
			...
			if (s1.equals("") || s2.equals("") || s3.equals("") || s4.equals("")) continue;
			addStars(map, s1, s2);
			addStars(map, s1, s3);
			addStars(map, s1, s4);
			addStars(map, s2, s3);
	...
	public List<String> getTopMovies(int top_k, String by) {
		...
		for (List<String> item : this.datas) {
			if (item.get(num).equals("") || item.get(1).equals("")) continue;
			int time = 0;
			if (num == 4)  time = Integer.parseInt(item.get(num).substring(0, item.get(num).length() - 4));
			else time = item.get(num).length();
			String title = item.get(1);
			ans.add(Map.entry(time, title));
		}
		ans.sort((a, b) -> a.getKey().equals(b.getKey()) ? a.getValue().compareTo(b.getValue()) : Integer.compare(-a.getKey(), -b.getKey()));
		List<String> kk = new ArrayList<>();
		for (int i = 0; i < top_k; i++) kk.add(ans.get(i).getValue());
		return kk;
	}
	
	public void addRating(Map<String, List<Float>> map, String star, Float rate) {
		if (map.containsKey(star)) map.get(star).add(rate);
		else {
	...
	public List<String> getTopStars(int top_k, String by) {
		...
		for (List<String> item : this.datas) {
			if (item.size() == 15 && num == 15) continue;
			String s1 = item.get(10);
			String s2 = item.get(11);
			String s3 = item.get(12);
			String s4 = item.get(13);
			if (item.get(num).equals("")) continue;
			if (s1.equals("") || s2.equals("") || s3.equals("") || s4.equals("")) continue;
	...
	public List<String> searchMovies(String genre, float min_rating, int max_runtime) {
		List<String> ans = new ArrayList<>();
		for (List<String> item : this.datas) {
			if (item.get(5).equals("") || item.get(6).equals("") || item.get(4).equals("") || item.get(1).equals("")) continue;
			if (item.get(5).contains(genre) && Double.parseDouble(item.get(6)) >= min_rating && Integer.parseInt(item.get(4).substring(0, item.get(4).length() - 4)) <= max_runtime) ans.add(item.get(1));
		}
